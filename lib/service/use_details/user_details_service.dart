  ...
  Future<UserDetails> userdetails({required String token}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ApiException("Bad request with status code: ${response.statusCode}");
      }
    } on DioError catch (e) {
      if (e.response != null) {
        // The server responded with a non-2xx status code
        throw ApiException('Request failed with status: ${e.response?.statusCode}, data: ${e.response?.data}');
      } else {
